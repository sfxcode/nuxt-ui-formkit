...
import { nuxtUIRadioGroupDefinition } from '../../../src/runtime/formkit/definitions/input'
...
describe('FURadioGroup name prop', () => {
  it('forwards the FormKit field name onto the underlying Nuxt UI component (pre-existing binding was silently broken - context.name never worked)', async () => {
    const wrapper = await mountSuspended(FormKit, {
      props: { type: 'form' },
      slots: {
        default: () => [h(FormKit, { type: 'nuxtUIRadioGroup', name: 'my-distinctive-name', options: ['a', 'b'] })],
      },
      attachTo: document.body,
      global: { plugins: [[plugin, defaultConfig({ inputs: { nuxtUIRadioGroup: nuxtUIRadioGroupDefinition } })]] },
    })
