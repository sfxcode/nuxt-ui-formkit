...
import { nuxtUIInputDefinition, nuxtUITextareaDefinition } from '../../../src/runtime/formkit/definitions/input'
...
describe('FUTextarea blur wiring', () => {
  it('reveals the validation message only after a real DOM blur, not before', async () => {
    const wrapper = await mountFormKit({
      type: 'nuxtUITextarea',
      value: '',
      inputs: { nuxtUITextarea: nuxtUITextareaDefinition },
      props: { validation: 'required' },
    })
    await settle()
