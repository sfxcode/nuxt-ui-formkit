...
describe('FUInputMenu name prop', () => {
  it('forwards the FormKit field name onto the underlying Nuxt UI component', async () => {
    const wrapper = await mountSuspended(FormKit, {
      props: { type: 'form' },
      slots: {
        default: () => [h(FormKit, { type: 'nuxtUIInputMenu', name: 'my-distinctive-name', options: ['a', 'b'] })],
      },
      attachTo: document.body,
      global: { plugins: [[plugin, defaultConfig({ inputs: { nuxtUIInputMenu: nuxtUIInputMenuDefinition } })]] },
    })
