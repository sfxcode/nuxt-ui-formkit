...
import { nuxtUIOutputLinkDefinition } from '../../../src/runtime/formkit/definitions/output'
  ...
  it('merges ui.root into the rendered container, alongside color-derived classes', async () => {
    const wrapper = await mountSuspended(FormKit, {
      props: {
        ...
        id: 'output-link-ui-test',
        modelValue: 'https://example.com',
        color: 'info',
        ui: { root: 'my-distinctive-root-class' },
      },
      global: {
        plugins: [[plugin, defaultConfig({ inputs: { nuxtUIOutputLink: nuxtUIOutputLinkDefinition } })]],
      },
    })
    activeWrapper = wrapper
    await settle()

    const container = wrapper.find('#output-link-ui-test')
    expect(container.exists()).toBe(true)
    expect(container.classes()).toContain('my-distinctive-root-class')
    expect(container.classes()).toContain('text-info')
  })
})
